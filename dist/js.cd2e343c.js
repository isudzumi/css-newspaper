...
},{"_css_loader":"../node_modules/parcel-bundler/src/builtins/css-loader.js"}],"../node_modules/css-regions-polyfill/bin/css-regions-polyfill.js":[function(require,module,exports) {
...
!function () {
	...
	var require = function () {
		var modules = {};var require = function (m) {
			return modules[m];
		};require.define = function (m) {
			modules[m] = module.exports;module.exports = {};
		};return require;
	}();
	...
	module.exports = function (window, document) {
		...
		var domEvents = {
			...
			cloneMouseEvent: function cloneMouseEvent(e) {
				...
				evt.initMouseEvent(e.type, e.canBubble || e.bubbles, e.cancelable, e.view, e.detail, e.screenX, e.screenY, e.clientX, e.clientY, e.ctrlKey, e.altKey, e.shiftKey, e.metaKey, e.button, e.relatedTarget);
				return evt;
			},
					...
					try {
						if (e[prop] != ne[prop] && e[prop] != e.target) {
							try {
								...
								Object.defineProperty(ne, prop, { get: function () {
										return e[prop];
									} });
							}
						}
		...
		domEvents.EventTarget.prototype.dispatchEvent = function (event_or_type) {
			...
			function setUpTarget(e, v) {
				try {
					Object.defineProperty(e, "target", { get: function () {
							return v;
						} });
				} catch (ex) {} finally {
	...
	module.exports = function (window, document) {
		...
		function TokenListToCSSString(sep) {
			if (sep) {
				...
				return this.asCSSString || (this.asCSSString = this.map(function (o) {
					return o.toCSSString();
		...
		function normalizeInput(input) {
			...
			if (input.length !== undefined) return new TokenStream(input);else throw SyntaxError(input);
		}
	...
	module.exports = function (window, document) {
		...
		function AnimationFrameEventStream(options) {
			...
			EventStream.call(this, function connect(yieldEvent) {
		...
		function MouseEventStream() {
			...
			// start the event stream
			EventStream.call(this, function connect(newYieldEvent) {
		...
		if ("MutationObserver" in window) {
			DOMUpdateEventStream = function DOMUpdateEventStream(options) {
				...
				EventStream.call(this, function connect(yieldEvent) {
					if (config) {
						observer = new MutationObserver(yieldEvent);observer.observe(target, config);target = null;config = null;
					}
				}, function disconnect() {
		...
		function CompositeEventStream(stream1, stream2) {
			...
			// fields
			var yieldEvent = null;var s1 = false,
			    s2 = false;
		...
		function querySelectorLive(selector, handler, root) {
			...
			var loop = function loop(eventStream) {
				...
				oldElms = oldElms.filter(function (e) {
					...
					var isStillInDocument = e === document.documentElement || document.documentElement.contains(e);
		...
		var cssCascade = {
						...
						for (var r = rules.length; r--;) {
							...
							if (rule instanceof cssSyntax.StyleRule) {
								...
								for (var sr = subrules.length; sr--;) {
									...
									try {
										if (element.matches) isMatching = element.matches(selector);else if (element.matchesSelector) isMatching = element.matchesSelector(selector);else if (element.oMatchesSelector) isMatching = element.oMatchesSelector(selector);else if (element.msMatchesSelector) isMatching = element.msMatchesSelector(selector);else if (element.mozMatchesSelector) isMatching = element.mozMatchesSelector(selector);else if (element.webkitMatchesSelector) isMatching = element.webkitMatchesSelector(selector);else {
											throw new Error("no element.matches?");
										}
									} catch (ex) {
										debugger;setImmediate(function () {
											throw ex;
										});
									}
			...
			getSpecifiedStyle: function getSpecifiedStyle(element, cssPropertyName, matchedRules) {
				...
				if (element.currentStyle && !window.opera) {
					...
					var rules = matchedRules || (cssPropertyName in cssCascade.monitoredProperties ? element.myMatchedRules || [] : cssCascade.findAllMatchingRules(element));
			...
			startMonitoringProperties: function startMonitoringProperties(properties, handler) {
				...
				for (var i = properties.length; i--;) {
					...
					var handlers = cssCascade.monitoredProperties[property] || (cssCascade.monitoredProperties[property] = []);
	...
	module.exports = function (window, document) {
		...
		var cssBreak = {
			...
			isSingleLineOfTextComponent: function (element, elementStyle, elementDisplay, elementPosition, isReplaced) {
				...
				return (elementDisplay === "inline-block" || elementDisplay === "inline-table" || elementDisplay === "inline-flex" || elementDisplay === "inline-grid"
				// TODO: more
			...
			isLinePreBreakingElement: function (element, elementStyle, elementDisplay, elementPosition) {
				...
				return breakBefore == "region" || breakBefore == "all" || cssBreak.isLineBreakingElement(element, elementStyle, elementDisplay, elementPosition);
			},
			...
			isLinePostBreakingElement: function (element, elementStyle, elementDisplay, elementPosition) {
				...
				return breakAfter == "region" || breakAfter == "all" || cssBreak.isLineBreakingElement(element, elementStyle, elementDisplay, elementPosition);
			},
			...
			isHiddenOverflowing: function isHiddenOverflowing(element, elementOverflow) {
				...
				return elementOverflow == "hidden" && element.offsetHeight != element.scrollHeight // trust me that works
				;
			},
	...
	Range.prototype.myMoveOneCharRight = function () {
		...
		if (r.startOffset < max) {
			...
			} else if (nextSibling && nextSibling.nodeType == nextSibling.TEXT_NODE && nextSibling.nodeValue != '') {
				// todo: lookup value
	...
	module.exports = function (window, document) {
		...
		var cssRegionsHelpers = window.cssRegionsHelpers = {
						...
						case 1:
							...
							var importPseudo = function (node1, node2, pseudo) {
								...
								if (pseudoStyle.content != 'none') {
									...
									stylesheet.textContent = '[data-css-regions-fragment-of="' + node1.getAttribute('data-css-regions-fragment-source') + '"]' + ':not([data-css-regions-starting-fragment]):not([data-css-regions-special-starting-fragment])' + ':' + pseudo + '{' + node2style + "}";
			...
			retargetEvent: function retargeEvent(node2, e) {
				...
				var node1 = node2.cssRegionsFragmentSource || (node2.cssRegionsFragmentSource = document.querySelector('[data-css-regions-fragment-source="' + node2.getAttribute('data-css-regions-fragment-of') + '"]'));
	...
	module.exports = function (window, document, cssRegions) {
		...
		cssRegions.Flow = function NamedFlow(name) {
			...
			this.name = name;Object.defineProperty(this, "name", { get: function () {
					return name;
				} });
		...
		cssRegions.Flow.prototype.addToContent = function (element) {
			...
			var treeWalker = document.createTreeWalker(document.documentElement, NodeFilter.SHOW_ELEMENT, function (node) {
		...
		cssRegions.Flow.prototype.generateContentFragment = function () {
			...
			//
			cssRegionsHelpers.transformFragmentSourceToFragments(Array.prototype.slice.call(fragment.childNodes, 0));
		...
		cssRegions.Flow.prototype.relayout = function () {
			...
			if (This.relayoutInProgress) {
				This.restartLayout = true;return;
			}
		...
		cssRegions.Flow.prototype._relayout = function (data) {
			...
			try {
				...
				if (This.failedLayoutCount < 7) {
					...
					This.failedLayoutCount = 0;This.relayoutScheduled = false;This.relayoutInProgress = false;This.restartLayout = false;
				}
			}
		};
		...
		cssRegions.Flow.prototype.relayoutIfSizeChanged = function () {
			...
			for (var i = rs.length; i--;) {
				if (rs[i].offsetHeight !== rs[i].cssRegionsLastOffsetHeight || rs[i].offsetWidth !== rs[i].cssRegionsLastOffsetWidth) {
		...
		cssRegions.Flow.prototype.addEventListenersTo = function (nodes) {
			var This = this;if (nodes instanceof Element) {
				nodes = [nodes];
			}
		...
		var cssRegions = {
			...
			layoutContent: function (regions, remainingContent, callback, startTime) {
				...
				if (!remainingContent.hasChildNodes()) {
					...
					var dummyCallback = { ondone: function () {}, onprogress: function (f) {
							f();
						} };
			...
			extractOverflowingContent: function (region, dontOptimize) {
				...
				try {
					var r = dontOptimize ? document.createRange() : document.caretRangeFromPoint(pos.left + sizingW - 1, pos.top + sizingH - 1);
				} catch (ex) {
					try {
    ...
    cssConsole.log(pos.bottom)*/
     	...
     	computedBCR: rect
				...
				var current = r.endContainer;if (current.hasChildNodes()) {
					if (r.endOffset > 0) {
				...
				if (r.endOffset == r.endContainer.childNodes.length && r.endContainer !== region) {
					...
					if (typeof borderCut === "number" && borderCut !== 0) {
						...
						var hasBottomRadius = parseInt(endContainerStyle.borderBottomLeftRadius) > 0 || parseInt(endContainerStyle.borderBottomRightRadius) > 0;
			...
			enablePolyfill: function enablePolyfill() {
				...
				cssCascade.startMonitoringProperties(["break-before", "break-after"], { onupdate: function (element) {
...
require('../../node_modules/css-regions-polyfill/bin/css-regions-polyfill.js');
},{"../css/main.css":"css/main.css","../../node_modules/css-regions-polyfill/bin/css-regions-polyfill.js":"../node_modules/css-regions-polyfill/bin/css-regions-polyfill.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
//# sourceMappingURL=/js.cd2e343c.map